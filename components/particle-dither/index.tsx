  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center justify-between">
        <span className="text-[9px] font-medium uppercase tracking-[0.14em] text-zinc-500">{label}</span>
        <span className="text-[10px] font-mono text-zinc-400 tabular-nums">{fmt(value)}</span>
      </div>
      <input
        type="range" min={min} max={max} step={step} value={value}
        onChange={(e) => onChange(Number(e.target.value))}
        className="w-full h-px appearance-none cursor-pointer bg-zinc-700 rounded accent-white"
      />
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <button
        ...
        className={`relative h-4 w-7 rounded-full transition-colors ${value ? "bg-white" : "bg-zinc-700"}`}
      >
        <span className={`absolute top-0.5 h-3 w-3 rounded-full shadow transition-transform ${value ? "translate-x-3.5 bg-black" : "translate-x-0.5 bg-zinc-400"}`} />
      </button>
    </div>
  );
}
...
function ParticleDitherFull() {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="absolute top-0 inset-x-0 flex items-center justify-between px-5 h-10 border-b border-zinc-800/60 z-10" style={{ background: "rgba(10,10,12,0.85)", backdropFilter: "blur(8px)" }}>
        <div className="flex items-center gap-2.5">
          <span className="text-[9px] font-bold uppercase tracking-[0.18em] text-zinc-600">Experiment 04</span>
          <span className="text-zinc-800">·</span>
          <span className="text-[11px] font-medium text-zinc-400">Particle Dither</span>
        </div>
        <div className="flex items-center gap-3">
          {error && <span className="text-[10px] text-red-400">{error}</span>}
          <span className="text-[10px] font-mono text-zinc-600 tabular-nums">{dotCount.toLocaleString()} dots</span>
          <button
            onClick={undo} disabled={!canUndo} title="Undo (⌘Z)"
            className="flex items-center justify-center w-6 h-6 rounded text-zinc-600 hover:text-zinc-300 hover:bg-white/5 transition-colors disabled:opacity-25 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 rounded text-[10px] font-medium uppercase tracking-wider text-zinc-500 hover:text-zinc-200 hover:bg-white/5 transition-colors"
          >
          ...
          <button
            onClick={resetSample}
            className="text-[10px] font-medium uppercase tracking-wider text-zinc-600 hover:text-zinc-400 transition-colors"
          >
            Reset
          </button>
        </div>
      ...
      <div className="absolute bottom-4 right-4 z-10 flex flex-col items-end gap-1">
        <button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[9px] font-medium uppercase tracking-widest transition-colors border border-zinc-800/80 text-zinc-600 hover:text-zinc-300 hover:border-zinc-700"
        ...
        {panelOpen && (
          ...
          >
            ...
            <div className="border-t border-zinc-800/60 pt-3 flex flex-col gap-3.5">
              <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-zinc-600">Physics</p>
            ...
            <div className="border-t border-zinc-800/60 pt-3 flex flex-col gap-3.5">
              <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-zinc-600">Render</p>
              ...
              <div className="flex items-center justify-between">
                <span className="text-[9px] font-medium uppercase tracking-[0.14em] text-zinc-500">BG Color</span>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono text-zinc-600 tabular-nums">{renderCfg.backgroundColor ?? "#000000"}</span>
                  <label className="relative cursor-pointer">
                    <div className="w-5 h-4 rounded border border-zinc-700" style={{ background: renderCfg.backgroundColor ?? "#000000" }} />
                    <input
                      type="color"
            ...
            <div className="border-t border-zinc-800/60 pt-3 flex flex-col gap-3.5">
              <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-zinc-600">Wave</p>
            ...
            <div className="border-t border-zinc-800/60 pt-3 flex flex-col gap-2.5">
              <p className="text-[9px] font-bold uppercase tracking-[0.15em] text-zinc-600">Presets</p>
              <div className="flex gap-1.5">
                <input
                  ...
                  placeholder="Name…"
                  className="flex-1 min-w-0 h-6 px-2 rounded text-[10px] bg-zinc-900 border border-zinc-800 text-zinc-300 placeholder-zinc-700 outline-none focus:border-zinc-600 transition-colors"
                />
                <button
                  onClick={savePreset}
                  className="h-6 px-2.5 rounded text-[9px] font-medium uppercase tracking-wider bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-zinc-200 transition-colors"
                >
                  Save
                </button>
              </div>
                  ...
                  {presets.map((p) => (
                    <div key={p.id} className="flex items-center gap-1.5">
                      <button
                        ...
                        className="flex-1 min-w-0 text-left text-[10px] text-zinc-400 hover:text-zinc-200 truncate transition-colors"
                      >
                        {p.name}
                      </button>
                      <button
                        onClick={() => deletePreset(p.id)}
                        className="text-zinc-700 hover:text-red-400 transition-colors text-[11px] leading-none shrink-0"
                      >
                        ×
                      </button>
                    </div>
      ...
      <div className="absolute bottom-4 left-4 z-10 pointer-events-none">
        <p className="text-[9px] font-medium uppercase tracking-[0.14em] text-zinc-700">Move · Click · Drag JSON</p>
      </div>
    </div>
  );
}
